/* =======================================================================
   LED Backpack Animator v1.0 — app.js (complete)
   - Preview/edit toolbar
   - Background presets + solid + upload (PNG/JPG/GIF)
   - Centered canvas, fit/zoom
   - Words, lines, emojis (scalable), manual drag, multi-select
   - Real-time animations with conflict warnings
   - Autosize (Word) + Autosize (Line) — same logic, new labels
   - GIF render (preview image + download in new tab)
...
const autoSizeWordChk=$("#autoSize");     // renamed label: Autosize (Word)
const autoSizeLineChk=$("#autoSizePerLine"); // renamed label: Autosize (Line)
...
on(bgUpload,"change",e=>{
  ...
  im.src=url; // PNG/JPG/GIF — animated GIFs will play in <img>
});
...
// click canvas in preview => go back to edit
...
function autoSizeAllIfOn(){
    ...
    if (autoSizeLineChk?.checked){
      ...
      for(let test=s; test>=6; test-=0.5){
        ...
        for(const w of line.words){
          ...
          // estimate width for text at test size
          if(!w.emoji){
  ...
  if (wSel && !wSel.emoji) fontSizeInp.value=Math.round(wSel.size||defaults.size);
}
...
function checkConflicts(anims){
  ...
  const conflicts=[];
  // slide + scroll + slideaway + zoom can conflict positionaly
  ...
  // strobe + flicker heavy alpha with fadeout
...
function buildAnimationsUI(){
  ...
  ANIMS.forEach(def=>{
    ...
    // checked if doc.anims has this def (panel working set), but if a word is selected with custom anims, mirror that
    ...
    Object.entries(def.params).forEach(([k,v])=>{
      ...
      on(inp,"input",()=>{
        // update panel pack or selected word pack
...
on(previewBtn,"click",()=>{ setMode("preview"); /* ensure preview image updates */ if(gifPreviewImg){ gifPreviewImg.src=""; gifPreviewImg.classList.add("hidden"); } });
...
/* =======================================================
   EMOJI PICKER (tabs, search, grid) + insertion
======================================================= */
...
function filterEmoji(){
  ...
  renderEmojiGrid(list.slice(0,1200)); // cap for perf
}
...
function init(){
  ...
  // default pill: Font open; others hidden (HTML sets open on Font)
  autoSizeWordChk.checked = true;     // Autosize (Word)
  autoSizeLineChk.checked = true;     // Autosize (Line)
  autoSizeAllIfOn(); render();

  // start in Edit by default (switch to preview if you prefer)
  setMode("edit");
}
init();
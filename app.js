...
/* =======================================================
   ANIMATIONS DEFINITIONS (Omitted for brevity, assumed correct)
======================================================= */
...
// ... (ANIMS array and resolveWordAnims functions omitted for space but assumed to be in your file)
...
function animatedProps(base, word, t, totalDur){
  ...
  // (All animation logic omitted for space but assumed correct in your file)
...
function render(t=0,totalDur=seconds()){
  ...
  // FIX: Only draw solid background if specified
  ...
  doc.lines.forEach((line,li)=>{
    ...
    line.words.forEach((w,wi)=>{
      ...
      // Emoji
      if(w.emoji){
        // ... (Emoji rendering logic omitted for space but assumed correct)
      ...
      // (Gradient/PerChar logic omitted for space but assumed correct)

      ctx.fillStyle=fillStyle;
      if(props.perChar && txt.length){
        // ... (Per char logic omitted for space but assumed correct)
      }else{
...
function drawSelectionBox(x,y,w,h,isMulti){ 
  ...
  if(!isMulti){ // draw delete handle only for single selection
    ...
    selected.handleBounds = { x:dx, y:dy, w:HANDLE_SIZE, h:HANDLE_SIZE };
  } else {
    selected.handleBounds = null; // Clear bounds if multi-select
  }
  ctx.restore(); 
}

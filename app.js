...
function bindBtn(id, ev, fn){ const el=$(id); if(el) el.addEventListener(ev, fn); }
function setVal(id, val){ const el=$(id); if(el) el.value=val; }
function setText(id, txt){ const el=$(id); if(el) el.textContent=txt; }

  const onId = (id, evt, fn)=>{ const el=$(id); if(el) el.addEventListener(evt, fn); };
  const showErr = (msg)=>{ const b=document.getElementById('errorBanner'); if(!b) return; b.textContent=msg; b.classList.remove('hidden'); setTimeout(()=>b.classList.add('hidden'), 5000); };
  ...
  onId('undoBtn',('click', undo);
  onId('redoBtn',('click', redo);
  ...
  const TILES=[
  {id:'Preset_A', label:'Wheelie 96×128', file:'assets/Preset_A.png', mode:'both'},
  {id:'Preset_B', label:'2 Up 96×128',    file:'assets/Preset_B.png', mode:'both'},
  ...
  function selectTile(t){
    $('solidWrap') && $('solidWrap').classList.add('hidden');
    $('customWrap') && $('customWrap').classList.add('hidden');
    if(t.id==='SOLID'){
      $('solidWrap') && $('solidWrap').classList.remove('hidden');
      S.bg={type:'solid',color:S.bg.color||'#000000',img:null};
    }else if(t.id==='CUSTOM'){
      $('customWrap') && $('customWrap').classList.remove('hidden');
    }else{
      const img=new Image();
      img.onload=()=>{ S.bg={type:'preset',name:t.id,img}; draw(); };
      img.onerror=()=>{ showErr('Missing asset '+t.file); S.bg={type:'solid',color:'#0d1320',img:null}; draw(); };
      img.src=t.file;
    }
    highlightSelected(); draw();
  }

  onId('solidPicker',('input', e=>{ S.bg={type:'solid',color:e.target.value,img:null}; draw(); });
  onId('customFile',('change', e=>{
...
onId('addLine',('click',()=>{ pushHistory(); 
...
onId('addWord',('click',()=>{ pushHistory(); 
    if(S.lines.length===0){ S.lines.push([{text:'WORD', color:'#ffffff'}]); S.active.line=0; S.active.word=0; ensureLineMeta(); S.lineMeta[0].align='center'; computeLayout(); draw(); syncInspector(); return; }
  ...
  onId('uploadPresetBtn',('click',()=> $('uploadPreset').click());
  onId('uploadPreset',('change',(e)=>{
    const f=e.target.files?.[0]; if(!f) return;
  ...
  onId('ownerPresetA',('click', ()=>{ applyOwnerPreset('A'); hideOwnerModal(); });
  onId('ownerPresetB',('click', ()=>{ applyOwnerPreset('B'); hideOwnerModal(); });
  onId('ownerClose',('click', hideOwnerModal);
  ...
  onId('ownerKeyBtn',('click', unlockOwner);
  onId('ownerKeyInput',('keydown', (e)=>{ if(e.key==='Enter'){ unlockOwner(); }});
  onId('openOwnerPresets',('click', showOwnerModal);
  onId('ownerClose',('click', hideOwnerModal);
  onId('ownerPresetA',('click', ()=>{ applyOwnerPreset('A'); hideOwnerModal(); });
  onId('ownerPresetB',('click', ()=>{ applyOwnerPreset('B'); hideOwnerModal(); });
